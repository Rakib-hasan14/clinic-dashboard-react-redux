...
export const fetchDoctorPageData = (position,sort,fromDate,toDate,search,status) => async(dispatch) => {
    const response = await axios.get(`https://clinic-management-dashboard-backend-rakib-hasan14.vercel.app/clinic/doctors?roll=${position}&accountStatus=${status}&sorting=${sort}&fromDate=${fromDate}&toDate=${toDate}&searchKey=${search}`)
...
export const fetchSingleDoctorPageData = (id) => async(dispatch) => {
    const response = await axios.get(`https://clinic-management-dashboard-backend-rakib-hasan14.vercel.app/clinic/doctors/single-details?id=${id}`)
