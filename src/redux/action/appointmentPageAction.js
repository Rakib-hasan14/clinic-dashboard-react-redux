...
export const fetchAppointPageData = (category,status,doctor,date) => async(dispatch) => {
    const response = await axios.get(`https://clinic-managements.herokuapp.com/clinic/appointment?category=${category}&status=${status}&doctor=${doctor}&date=${date}`)

    dispatch({type: actionTypesAppointment.GET_APPOINT_DATA, payload: response.data})
}  

export const fetchCategoryData = () => async(dispatch) => {
    const response = await axios.get(`https://clinic-managements.herokuapp.com/clinic/appointment/category`)
...
export const fetchDoctorData = () => async(dispatch) => {
    const response = await axios.get(`https://clinic-managements.herokuapp.com/clinic/doctors?accountStatus=active&sorting=desc`)
...
export const fetchPateintData = (search) => async(dispatch) => {
    const response = await axios.get(`https://clinic-managements.herokuapp.com/clinic/pateints?status=active&searchKey=${search}`)
...
export const makeDone = (id) => async(dispatch) => {
    const response = await axios.get(`https://clinic-managements.herokuapp.com/clinic/appointment/done?id=${id}`)

    dispatch({type: actionTypesAppointment.MAKE_DONE , payload: response.data})
}  

export const AddAppointAction = (data) => async(dispatch) => {
    const response = await axios.post(`https://clinic-managements.herokuapp.com/clinic/appointment/add`,data)
